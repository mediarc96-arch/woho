...
import { assertBookingParticipant } from '../policies/booking-access.policy';

@Injectable()
export class CancelBookingUseCase implements UseCase<BookingActionCommand, Booking> {
  constructor(
  ...
  async execute({ bookingId, actorId }: BookingActionCommand): Promise<Booking> {
    ...
    assertBookingParticipant(booking, tour, actorId);
